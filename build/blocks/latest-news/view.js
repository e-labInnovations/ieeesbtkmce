...
document.addEventListener("DOMContentLoaded", function (event) {
  document.querySelectorAll("#latest-news-section").forEach(latestNewsSection => {
    ...
    const slideDiv = latestNewsSection.querySelector(imagesClasses.default).parentElement;
    const newsItems = latestNewsSection.querySelectorAll(imagesClasses.default);
    ...
    const setImageClasses = () => {
      newsItems.forEach((newsItem, i) => {
        newsItem.classList.remove(imagesClasses.frontHidden, imagesClasses.front, imagesClasses.back, imagesClasses.backHidden, "hidden");
        if (i === activePos) {
        ...
        } else if (i === activePos - 1 || activePos === 0 && i === newsItems.length - 1) {
    ...
    const lnInit = () => {
      ...
      activeImg.parentElement.style.height = imagesDivWidth / 1.675 * 1.1764705882352942 + "px";
    };
    lnInit();
