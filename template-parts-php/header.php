<nav class="top-0 z-50 border-gray-200 bg-white px-10">
    <div class="mx-auto flex max-w-screen-xl flex-wrap items-center justify-between pt-6">
        <a href="<?php echo get_home_url(); ?>" class="flex items-center space-x-3 rtl:space-x-reverse">
            <?php if (function_exists("the_custom_logo")) {
        $custom_logo_id = get_theme_mod("custom_logo");
        $logo = wp_get_attachment_image_src($custom_logo_id, "full");
      } ?>
            <img src="<?php echo $logo[0]; ?>" class="h-12" alt="<?php echo get_bloginfo(
  "name"
); ?>" />
      ...
      "menu_class" =>
        "flex flex-col font-medium p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white",
